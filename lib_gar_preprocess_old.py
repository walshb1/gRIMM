...
def get_dk_over_k_from_file(path):
...
def gar_preprocessing(inputs,intermediates):
    ...
    #agg data
    print(inputs)
    gar_aal_data = pd.read_csv(inputs+'/GAR15 results feb 2016_AALmundo.csv',encoding='utf-8')
    ...
    frac_value_destroyed_gar.to_csv(intermediates+'/frac_value_destroyed_gar.csv',header=True)

    #Check. should be 0
    #print((exposed_value_GAR - gar_exposed_value).abs().sort_values(ascending=True).head(5))

    capital_losses_from_GAR_events = pd.read_csv(intermediates+'/capital_losses_from_GAR_events.csv', index_col=["country","hazard","rp"], squeeze=True)

    frac_cap_distroyed_from_events = capital_losses_from_GAR_events/ev_gar.median(level="country")
    frac_cap_distroyed_from_events.to_csv(intermediates+'/frac_cap_distroyed_from_events.csv', header=True)
    ...
    print('GAR preprocessing script: writing out '+intermediates+'/frac_value_destroyed_gar_completed.csv')
    frac_value_destroyed_gar_completed.to_csv(intermediates+'/frac_value_destroyed_gar_completed.csv', encoding="utf-8", header=True)
