...
if arcope:
    ...
    fas['hazard'] ='flood'
    fas = fas.set_index(['Country Name','hazard','rp'])['fa']

###apply \delta_K = f_a * V, and use destroyed capital from GAR data, and fa_threshold to recalculate vulnerability
frac_value_destroyed_gar = pd.read_csv(intermediate+"frac_value_destroyed_gar_completed.csv",dtype = {'rp':np.int32}, index_col=["country", "hazard", "rp"], squeeze=True);#\delta_K, Generated by pre_process\ GAR.ipynb
if arcope:
    comparison = frac_value_destroyed_gar.loc[['Peru','Colombia','Argentina'],'flood'].to_frame('gar')
    frac_value_destroyed_gar.update(fas)
    comparison['glofris'] = frac_value_destroyed_gar.loc[['Peru','Colombia','Argentina'],'flood']
    comparison.to_csv('arcope_exceedance_comparison.csv')
    if ssbn_test:
        comparison = pd.read_csv('arcope_exceedance_comparison_ssbn.csv',dtype={'rp':np.int32})
        comparison = comparison.drop(comparison.columns[['Unnamed' in col for col in comparison.columns]],axis =1 )
        ar_ssbn = comparison.set_index(['country','hazard','rp'])['ssbn_space']
        ar_ssbn.loc[['Argentina','Colombia'],'flood']
        frac_value_destroyed_gar.loc[['Argentina','Colombia'],'flood'] = ar_ssbn.loc[['Argentina','Colombia'],'flood']
        # if False:
        #     ar_ssbn = (pd.read_csv('../ssbn/pop_fa.csv').loc[7]/100).drop('pop')
        # else:
        #     ar_ssbn = pd.read_csv('../ssbn/spatial_fas.csv').sum().drop('Unnamed: 0')/100

        # ar_ssbn[1500] = ar_ssbn[1000]
        # ar_ssbn[2000] = ar_ssbn[1000]
